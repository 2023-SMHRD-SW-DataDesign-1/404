...
<!-- css 연결 -->
<link rel="stylesheet" href="./FeedEditStyle.css">
...
<body>
   ...
   <!-- 수정 1 -->
	...
	<c:set var="myFeed" value="${FeedDAO1.showMyFeeds()}"></c:set>
		<!-- 피드 번호 -->
		<input type="hidden"><%=session.getAttribute("feedNo")%>
		
		<!-- 피드 이미지 -->
		<div class="feed-img">
			<img src="../img2/${myFeed.feedImages}" alt="피드 이미지">
		</div>

		<!-- 수정한 텍스트 보낼 form 태그 -->
		<form action="UpdateFeedCon" method="post">
			<!-- 피드 텍스트 -->
			<div class="feed-text">
				<textarea name="feedText" placeholder="텍스트 입력">${myFeed.text}</textarea>
			</div>


			<!-- 피드 전송버튼(내 피드 상세페이지) -->
			<!-- ../SellerDetail/MyFeedDetail.jsp로 이동 -->
			<div class="submit">

				<button type="submit" formaction="">등록하기</button>
			</div>
		</form>
